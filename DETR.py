...
from sample_test_DETR import RTDETRVisualLogger # Import class vẽ box của Tú vào đây  
...
def on_fit_epoch_end(trainer):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            eval_model = RTDETR(trainer.last)
...
# ==============================================================================
class CustomTrainer(RTDETRTrainer):
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="Captcha_RTDETR",      # Tên Project trên Dashboard
        name="RTDETR_A100_Run1",       # Tên phiên chạy (cậu tự đặt cho ngầu)
        config={                       # Lưu lại cấu hình để sau này dễ xem lại
            "architecture": "RTDETRn",
            "imgsz": 320,
            "batch_size": 64,
            "epochs": 150
        }
    )
    visual_logger = RTDETRVisualLogger(val_dir=VAL_IMG_DIR, num_samples=10)
    trainer = CustomTrainer(overrides={
        "model": "rtdetr-l.pt",         # Load tạ gốc của YOLOv8n
        "data": "dataset.yaml",        # File config trỏ đến thư mục train/val
        "epochs": 150,
        "imgsz": 320,
        "batch": 64,
        "patience": 30,
        "project": "Captcha_RTDETR",

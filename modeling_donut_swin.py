...
class DonutSwinSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # ====================================================================
        # BẢN VÁ LỖI CỦA TÚ: NỘI SUY MA TRẬN ATTENTION (DYNAMIC WINDOW SIZE)
        # ====================================================================
        actual_seq_len = attention_scores.shape[-1]
        expected_seq_len = self.window_size[0] * self.window_size[1]

        if actual_seq_len != expected_seq_len:
            # Nếu ma trận bị lệch do ảnh nhỏ, ép ma trận Bias co dãn theo đúng kích thước thực tế
            relative_position_bias = relative_position_bias.unsqueeze(0) # Thêm chiều batch giả
            relative_position_bias = torch.nn.functional.interpolate(
                relative_position_bias,
                size=(actual_seq_len, actual_seq_len),
                mode="bicubic",
                align_corners=False
            )
            relative_position_bias = relative_position_bias.squeeze(0) # Xóa chiều batch giả đi
        # ====================================================================
